...
class MLPPolicy(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        ...
        with torch.no_grad():
            action_distribution = ptu.from_numpy(observation)
...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        loss = -torch.sum(action_log_prob*advantages)
        loss.backward()
        self.optimizer.step()
        ...
        if self.nn_baseline:
            ...
            nn_baseline_loss = self.baseline_loss(pred.squeeze(), q_values)
            nn_baseline_loss.backward()
