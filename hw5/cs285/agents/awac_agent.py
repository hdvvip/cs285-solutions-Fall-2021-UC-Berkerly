...
class AWACAgent(DQNAgent):
    ...
    def estimate_advantage(self, ob_no, ac_na, re_n, next_ob_no, terminal_n, n_actions=10):
        ...
        qa_t_values = self.exploitation_critic.q_net(ob_no)
        q_t_values = torch.gather(qa_t_values, 1, ac_na.unsqueeze(1)).squeeze(1)
        qa_tp1_values = self.exploitation_critic.q_net_target(next_ob_no)

        next_actions = self.exploitation_critic.q_net(next_ob_no).argmax(dim=1)
        q_tp1 = torch.gather(qa_tp1_values, 1, next_actions.unsqueeze(1)).squeeze(1)

        adv_n = re_n + self.rnd_gamma * q_tp1 * (1 - terminal_n) - q_t_values
